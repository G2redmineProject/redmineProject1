...
public interface MyPageService {
	 ...
	 * 내 페이지 전체 데이터 조합
	 *
	 * 🔹 설명 - 화면에 필요한 모든 데이터를 한 번에 구성한다. - 블록 목록 조회 - 블록 타입에 따라: ASSIGNED → 할당된 일감
	 * REGISTERED → 등록한 일감 NOTICE → 최근공지 CALENDAR → 주간 달력 WORKLOG → 작업내역 - 각각 필요한
	 * 데이터를 조회하여 Map 형태로 반환한다.
	 *
	 * 🔹 반환 구조 예시 { "blocks" : 블록 목록, "blockData" : 블록별 실제 데이터, "days" : 작업내역 조회 기간
	 * }
	 *
