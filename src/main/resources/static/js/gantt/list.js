...
(() => {
    ...
    const getFilteredDataWithHierarchy = (data, filters) => {
        ...
        const filteredIssues = data.filter(item => {
            ...
            // 프로젝트 상태
            if (filters.projectStatus && item.projectStatusName !== filters.projectStatus) {
                ok = false;
            }
    ...
    /* ========================================================================
       ...
       ======================================================================== */
    let cachedRawData = null; // 캐시 변수 추가   
    ...
    const fData = async (filters = {}) => {
        ...
        if (spinner) spinner.style.display = "flex"; // ① 로딩 시작

        try {
            // 캐시 없으면 API 호출, 있으면 재사용
            if (!cachedRawData) {
            ...
            // 관리자 체크
            ADMIN_PROJECTS = cachedRawData.adminProjects || [];
            ...
            if (!ganttData.data || ganttData.data.length === 0) {
                ...
                // 여기서 return 하지 말고 자연스럽게 finally로 가게 둡니다.
            } else {
                if (emptyEl) emptyEl.style.display = "none";
                if (ganttEl) ganttEl.style.visibility = "visible";

                // parse 도중 에러가 나도 spinner는 꺼져야 함
                try {
